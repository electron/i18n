...
* `options` Objekt (optional) 
  ...
  * `webPreferences` Object (optional) - Einstellungen der Funktionalität der Webseite. 
    * `devTools` Boolean (optional) - Gibt an ob die Entwicklerwerkzeuge aktiviert werden sollen. Falls dies auf `false` gesetzt ist, kann `BrowserWindow.webContents.openDevTools()` nicht verwendet werden um die Entwicklerwerkzeuge zu öffnen. Standard ist `true`.
    * `nodeIntegration` Boolean (optional) - Gibt an ob die Node Integration aktiviert ist. Standard ist `true`.
    * `nodeIntegrationsInWorker` Boolean (optional) - Gibt an ob die Node Integration in Web Workern aktiviert ist. Standard ist `false`. More about this can be found in [Multithreading](../tutorial/multithreading.md).
