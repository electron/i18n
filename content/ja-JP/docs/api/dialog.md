...
* `callback` Function (任意) 
  * `filePaths` String[] (任意) - ユーザーによって選択されたファイルパスの配列。このダイアログがキャンセルされた場合のこれは`未定義`になります。
...
* `callback` Function (任意) 
  * `filename` String (任意) このダイアログがキャンセルされた場合これは`未定義`になります。
  * `bookmark` String (任意)*macOS* *mas* - 保存されたファイルのセキュリティスコープのブックマークデータを含む Base64 エンコードされた文字列。 出力するために `securityScopedBookmarks` を有効にする必要があります。
...
`callback` と `browserWindow` の引数が渡された場合、このダイアログはこのプロセスをブロックしません。 このAPI 呼び出しは非同期です。そのため結果は`callback(response)`を介して渡されます。
