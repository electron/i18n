...
* `options` Object (任意) 
  ...
  * `webPreferences` Object (任意) - Webページの機能設定。 
    ...
    * `sandbox` Boolean (任意) - 設定された場合、これはウインドウと関連付けられているレンダラーのChromiumのOSレベルのサンドボックス互換で、Node.jsエンジンを無効化するサンドボックスとなります。 This is not the same as the `nodeIntegration` option and the APIs available to the preload script are more limited. Read more about the option [here](sandbox-option.md). **Note:** This option is currently experimental and may change or be removed in future Electron releases.
