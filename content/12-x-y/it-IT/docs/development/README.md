# Sviluppare Electron

Queste guide sono destinate alle persone che lavorano al progetto stesso di Electron. For guides on Electron app development, see [/docs/README.md](../README.md#guides-and-tutorials).

* [Codice di Condotta](https://github.com/electron/electron/blob/master/CODE_OF_CONDUCT.md)
* [Contribuire a Electron](https://github.com/electron/electron/blob/master/CONTRIBUTING.md)
* [Problemi](issues.md)
* [Richieste di pull](pull-requests.md)
* [Linee guida per la documentazione](coding-style.md#documentation)
* [Struttura della cartella del codice sorgente](source-code-directory-structure.md)
* [Stile Codice](coding-style.md)
* [Uso di clang-format nel codice C++](clang-format.md)
* [Using clang-tidy on C++ Code](clang-tidy.md)
* [Panoramica sul sistema di compilazione](build-system-overview.md)
* [Istruzioni per la compilazione (macOS)](build-instructions-macos.md)
* [Istruzioni per la compilazione (Windows)](build-instructions-windows.md)
* [Istruzioni per la compilazione (Linux)](build-instructions-linux.md)
* [Sviluppo di Chrome](chromium-development.md)
* [Sviluppo di V8](v8-development.md)
* [Prova](testing.md)
* [Debugging su Windows](debug-instructions-windows.md)
* [Debugging su macOS](debugging-instructions-macos.md)
* [Impostare Server Symbol nel Debugger](setting-up-symbol-server.md)
* [Patches](patches.md)
