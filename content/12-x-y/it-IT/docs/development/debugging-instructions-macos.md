...
* **Xcode**: Oltre ad Xcode, installa anche gli strumenti di linea di comando. Includono LLDB, il debugger predefinito in Xcode su macOS. Supporta il debugging in C, Objective-C e C++ su dispositivi e simulatori desktop e iOS.
...
LLDB è un potente strumento con una grande documentazione. To learn more about it, consider Apple's debugging documentation, for instance the [LLDB Command Structure Reference][lldb-command-structure] or the introduction to [Using LLDB as a Standalone Debugger][lldb-standalone].

Puoi anche vedere il fantastico [manuale e tutorial][lldb-tutorial] di LLDB che spiegherà scenari di debug più complessi.
