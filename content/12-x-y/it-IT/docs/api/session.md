...
### `session.fromPartition(partition[, opzioni])`
...
Emitted after an extension is loaded. This occurs whenever an extension is added to the "enabled" set of extensions. Questo include:
...
* `options` Object
  * `offline` Boolean (optional) - Whether to emulate network outage. Il valore predefinito è false.
