...
`Tray`는 [EventEmitter][event-emitter]이다.
...
* `options` Object
  ...
  * `title` String
  * `content` String
  * `largeIcon` Boolean (optional) - The large version of the icon should be used. 기본값은 `true`이다. Maps to [`NIIF_LARGE_ICON`][NIIF_LARGE_ICON].
  * `noSound` Boolean (optional) - Do not play the associated sound. 기본값은 `false`이다. Maps to [`NIIF_NOSOUND`][NIIF_NOSOUND].
  * `respectQuietTime` Boolean (optional) - Do not display the balloon notification if the current user is in "quiet time". 기본값은 `false`이다. Maps to [`NIIF_RESPECT_QUIET_TIME`][NIIF_RESPECT_QUIET_TIME].
