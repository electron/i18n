...
[Page Visibility API][page-visibility-api]의 동작방식은 다음과 같다. :
...
* `options` Object (optional)
  ...
  * `webPreferences` Object (optional) - Settings of web page's features.
    ...
    * `enableRemoteModule` Boolean (optional) - Whether to enable the [`remote`](remote.md) module. 기본값은 `false`이다.
    ...
    * `nativeWindowOpen` Boolean (optional) - Whether to use native `window.open()`. Defaults to `false`. Child windows will always have node integration disabled unless `nodeIntegrationInSubFrames` is true. **참고:** 이 옵션은 현재 실험적입니다.
    ...
    * `spellcheck` Boolean (optional) - Whether to enable the builtin spellchecker. 기본값은 `true`이다.
    * `enableWebSQL` Boolean (optional) - Whether to enable the [WebSQL api](https://www.w3.org/TR/webdatabase/). 기본값은 `true`이다.
    ...
    * `enablePreferredSizeMode` Boolean (optional) - Whether to enable preferred size mode. The preferred size is the minimum size needed to contain the layout of the document—without requiring scrolling. Enabling this will cause the `preferred-size-changed` event to be emitted on the `WebContents` when the preferred size changes. 기본값은 `false`이다.
...
**참고:** 웹 페이지의 제목은 네이티브 창의 제목과 다를 수 있습니다.
