...
`DownloadItem` Electron içindeki indirme öğesini temsil eden bir [EventEmitter][event-emitter]'dir. O `oturumun` `will-download` olayı içinde kullanılır ve kullanıcıların indirilen öğeyi kontrol etmesine izin verir.
...
API, yalnızca oturumun `will-download` geri arama işlevinde kullanılabilir. If `path` doesn't exist, Electron will try to make the directory recursively. If user doesn't set the save path via the API, Electron will use the original routine to determine the save path; this usually prompts a save dialog.
