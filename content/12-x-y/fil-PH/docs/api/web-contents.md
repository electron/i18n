...
`webContents` is an [EventEmitter][event-emitter]. Ito ay responsable sa pag-render at pagkontrol sa isang web page at bagay na ari-arian ng [`BrowserWindow`](browser-window.md). Isang halimbawa ng pag-access sa `webContents` bagay:
...
This event is like `did-finish-load` but emitted when the load failed. Ang buong listahan ng mga error code at ang kanilang mga kahulugan ay magagamit [dito](https://source.chromium.org/chromium/chromium/src/+/master:net/base/net_error_list.h).
...
* `input` Object - Input properties.
  ...
  * `susi` Pisi - Katumbas ng [KeyboardEvent.key][keyboardevent].
  * `code` Pisi - Katumbas ng [KeyboardEvent.code][keyboardevent].
  * `isAutoRepeat` Boolean - Katumbas ng [KeyboardEvent.ulitin][keyboardevent].
  * `isComposing` Boolean - Equivalent to [KeyboardEvent.isComposing][keyboardevent].
  * `shift` Boolean - Katumbas ng [KeyboardEvent.shiftKey][keyboardevent].
  * `kontrol` Boolean - Katumbas ng [KeyboardEvent.controlKey][keyboardevent].
  * `alt` Boolean - Katumbas ng [KeyboardEvent.altKey][keyboardevent].
  * `meta` Boolean - Katumbas ng [KeyboardEvent.metaKey][keyboardevent].
...
Upang mapigilan lamang ang mga shortcut sa menu, gamitin ang [`setIgnoreMenuShortcuts`](#contentssetignoremenushortcutsignore):
