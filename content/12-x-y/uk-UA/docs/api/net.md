...
Модуль `net` це API для запитів клієнта через HTTP(S). Це подібно модулям [HTTP](https://nodejs.org/api/http.html) та [HTTPS](https://nodejs.org/api/https.html) в Node.js, але використовує власну мережеву бібліотеку Chromium в противагу власним реалізаціям від Node.js, обіцяючи кращу підтримку для веб проксі. It also supports checking network status.
