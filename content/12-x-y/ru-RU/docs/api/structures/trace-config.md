...
* `enable_argument_filter` boolean (опционально) - если значение true, данные событий отфильтруются в соответствии с определенным списком событий, которые были проверены вручную, чтобы не включать PII. Более подробно смотрите [реализацию в Chromium][trace_event_args_whitelist.cc].
