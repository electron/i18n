...
Elektron memungkinkan pengembang untuk menonaktifkan berbagai fitur keamanan kontrol proses renderer. Dalam kebanyakan kasus, pengembang tidak perlu menonaktifkan salah satu fitur tersebut - dan Anda harus karena itu tidak memungkinkan konfigurasi yang berbeda untuk [`<webview>`][webview-tag]tag yang baru dibuat.

### Bagaimana?

Sebelum [`<webview>`][webview-tag]tag terpasang, elektron akan api acara `akan-melampirkan-webview` pada hosting `webContents`. Use the event to prevent the creation of `webViews` with possibly insecure options.
