...
`webContents` is an [EventEmitter][event-emitter]. Ini bertanggung jawab untuk kontrol dan mengendalikan halaman web dan properti objek [`Jendela Peramban`](browser-window.md). Contoh untuk mengakses objek `isi web`:
...
This event is like `did-finish-load` but emitted when the load failed. Daftar lengkap kode galat dan makna mereka tersedia [di sini](https://source.chromium.org/chromium/chromium/src/+/master:net/base/net_error_list.h).
...
* `input` Object - Input properties.
  ...
  * `kunci` String - setara dengan [KeyboardEvent.key][keyboardevent].
  * `kode` String - setara dengan [KeyboardEvent.code][keyboardevent].
  * `isAutoRepeat` Boolean - setara dengan [KeyboardEvent.repeat][keyboardevent].
  * `isComposing` Boolean - Equivalent to [KeyboardEvent.isComposing][keyboardevent].
  * `pergeseran` Boolean - setara dengan [KeyboardEvent.shiftKey][keyboardevent].
  * `kontrol` Boolean - setara dengan [KeyboardEvent.controlKey][keyboardevent].
  * `Alt` Boolean - setara dengan [KeyboardEvent.altKey][keyboardevent].
  * `meta` Boolean - setara dengan [KeyboardEvent.metaKey][keyboardevent].
...
Untuk hanya mencegah menu cara pintas, menggunakan [`setIgnoreMenuShortcuts`](#contentssetignoremenushortcutsignore):
...
<p spaces-before="0">Send an asynchronous message to the renderer process via <code>channel`, along with arguments. Arguments will be serialized with the [Structured Clone Algorithm][SCA], just like [`postMessage`][], so prototype chains will not be included. Sending Functions, Promises, Symbols, WeakMaps, or WeakSets will throw an exception.</p>
...
<code>channel`, along with arguments. Arguments will be serialized with the [Structured Clone Algorithm][SCA], just like [`postMessage`][], so prototype chains will not be included. Sending Functions, Promises, Symbols, WeakMaps, or WeakSets will throw an exception.</p>
