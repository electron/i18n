...
* **Xcode**: Selain Xcode, juga menginstal alat baris perintah Xcode. They include LLDB, the default debugger in Xcode on macOS. It supports debugging C, Objective-C and C++ on the desktop and iOS devices and simulator.
...
LLDB adalah alat yang ampuh dengan dokumentasi yang bagus. To learn more about it, consider Apple's debugging documentation, for instance the [LLDB Command Structure Reference][lldb-command-structure] or the introduction to [Using LLDB as a Standalone Debugger][lldb-standalone].

Anda juga dapat memeriksa LLDB ini fantastis [manual dan tutorial][lldb-tutorial], yang akan menjelaskan skenario debugging lebih kompleks.
