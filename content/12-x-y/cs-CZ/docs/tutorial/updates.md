...
Tým Electronu udržuje [update.electronjs.org][], bezplatnou a open-source webovou službu, kterou mohou Electron aplikace používat pro vlastní aktualizaci. Služba je určena pro elektronové aplikace, které splňují tato kritéria:
...
Nejjednodušší způsob, jak tuto službu používat, je instalace [update-electron-app][], modulu Node.js přednastaveného pro použití s update.electronjs.org.
...
Pokud potřebujete přizpůsobit svou konfiguraci, můžete [předat možnosti `update-electron-app`][update-electron-app] nebo [použít službu aktualizací přímo][update.electronjs.org].
...
- [Hazel][hazel] – aktualizace serveru pro soukromé nebo open-source aplikace, které mohou být nasazeny zdarma [Teď][now]. Táhne z [GitHub Releases][gh-releases] a využívá sílu GitHubu CDN.
- [Nuts][nuts] – také používá [GitHub Releases][gh-releases], ale aplikuje aktualizace na disku a podporuje soukromé repozitáře.
- [electron-release-server][electron-release-server] – poskytuje nástěnku pro zpracování vydání a nevyžaduje vydání pro původ na GitHubu.
- [Nucleus][nucleus] – kompletní aktualizační server pro Electron aplikace spravovaný Atlassianem. Podporuje více aplikací a kanálů; používá statické úložiště souborů k vytěžování nákladů na server.
