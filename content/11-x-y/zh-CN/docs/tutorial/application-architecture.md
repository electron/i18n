...
> #### 题外话：进程间通讯
> 
> 在 Electron 中，主进程和渲染进程之间的通信是通过 [`ipcRenderer`](../api/ipc-renderer.md) 和  [`ipcMain`](../api/ipc-main.md) 模块完成的。 There is also an FAQ entry on [how to share data between web pages][share-data].
...
由于进程之间可以进行沟通，渲染进程可以通过 IPC 呼叫主进程执行任务。
