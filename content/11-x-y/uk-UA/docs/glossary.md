...
ASAR це Atom Shell Archive Format. An [asar][asar] archive is a simple `tar`-like format that concatenates files into a single file. Electron може працювати з файлами архіву без його розпаковування.
...
Apple Disk Image (DMG) — це пакетний формат, який використовується в macOS. DMG files are commonly used for distributing application "installers". [electron-builder][] підтримує `dmg` формат як ціль побудови/компіляції.
...
IPC означає взаємообробне спілкування. Electron uses IPC to send serialized JSON messages between the [main][] and [renderer][] processes.
...
Acronym для Apple Mac App Store. For details on submitting your app to the MAS, see the [Mac App Store Submission Guide][].
...
Дивіться також [Використання Нативних Модулів Node][].

### NSIS

Nullsoft Scriptable Install System це керований скриптом інсталятор для Microsoft Windows. Випускається під комбінацією вільних прикладних ліцензій і гироко використовується як альтернатива комерційним пропрієтарним продуктам таким як InstallShield. [electron-builder][] підтримує NSIS як ціль побудови.
...
Процес — екземпляр комп'ютерної програми, яка виконується. Застосунки Electron які використовують [головний][] і один чи декілька процесів [рендерингу][] фактично виконують декілька програм одночасно.
...
The renderer process is a browser window in your app. Unlike the main process, there can be multiple of these and each is run in a separate process. Вони також можуть бути приховані.
...
Squirrel це фреймворк з відкритим кодом, що дозволяє застосункам Electron оновлюватися автоматично коли виходить нова версія. Дивіться [autoUpdater][] API для початку роботи з Squirrel.
...
`webview` tags are used to embed 'guest' content (such as external web pages) in your Electron app. They are similar to `iframe`s, but differ in that each webview runs in a separate process. Він не має таких самих прав як ваша веб-сторінка і вся взаємодія вашого застосунку та вставленого контенту буде асинхронною. Це захищає ваш додаток від нього.

[аддонами]: https://nodejs.org/api/addons.html
...
[main]: #main-process
[головний]: #main-process
[renderer]: #renderer-process
[рендерингу]: #renderer-process
[Використання Нативних Модулів Node]: tutorial/using-native-node-modules.md
[V8]: #v8
