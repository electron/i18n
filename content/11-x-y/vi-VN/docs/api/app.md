...
Emitted during [Handoff][handoff] before an activity from a different device wants to be resumed. You should call `event.preventDefault()` if you want to handle this event.
...
Emitted during [Handoff][handoff] when an activity from a different device fails to be resumed.
...
Emitted during [Handoff][handoff] after an activity from this device was successfully resumed on another one.
...
* `webContents` [WebContents](web-contents.md)
* `details` Object
  * `reason` String - The reason the render process is gone.  Giá trị khả thi:
...
* `details` Object
  * `type` String - Process type. Một trong những điều sau đây:
    ...
    * `Unknown`
  * `reason` String - The reason the child process is gone. Giá trị khả thi:
    * `clean-exit` - Process exited with an exit code of zero
    * `abnormal-exit` - Process exited with a non-zero exit code
    * `killed` - Process was sent a SIGTERM or otherwise killed externally
    * `crashed` - Process crashed
...
Ẩn tất cả các cửa sổ ứng dụng xuống mà không có minimize chúng.
...
Trả về `String` - thư mục ứng dụng hiện tại.
...
* `options` Object (optional)
  * `size` String
    * `small` - 16x16
    * `normal` - 32x32
    * `large` - 48x48 trên _Linux_, 32x32 trên _Windows_, không hỗ trợ trên _macOS_.

Returns `Promise<NativeImage>` - fulfilled with the app's icon, which is a [NativeImage](native-image.md).

Fetches a path's associated icon.

Trên _Windows_, có 2 loại biểu tượng:
...
Ghi đè lên tên ứng dụng hiện tại.
...
Xóa danh sách tài liệu gần đây.

### `app.setAsDefaultProtocolClient(protocol[, path, args])`

* `protocol` String - Tên của protocol của bạn, không chứa `://`. For example, if you want your app to handle `electron://` links, call this method with `electron` as the parameter.
* `path` String (optional) _Windows_ - The path to the Electron executable. Defaults to `process.execPath`
* `args` String[] (optional) _Windows_ - Arguments passed to the executable. Mặc định là một empty array
...
* `protocol` String - Tên của protocol của bạn, không chứa `://`.
* `path` String (optional) _Windows_ - Mặc định là `process.execPath`
* `args` String[] (optional) _Windows_ - Mặc định là một array rỗng
...
* `options` Object (optional)
  ...
  * `args` String[] (optional) _Windows_ - The command-line arguments to compare against. Mặc định là một empty array.
...
* `settings` Object
  ...
  * `args` String[] (optional) _Windows_ - The command-line arguments to pass to the executable. Mặc định là một empty array. Take care to wrap paths in quotes.
