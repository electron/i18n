...
* `enable_argument_filter` booleano (opzionale) - se true, filtra i dati dell'evento in base ad un elenco specifico di eventi che sono stati controllati manualmente affinché non includessero alcun PII. See [the implementation in Chromium][trace_event_args_whitelist.cc] for specifics.
