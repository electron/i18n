...
Per impostazione predefinita, `npm start` avrebbe eseguito lo script principale con Node.js. per far girare con Electron, puoi aggiungere uno script `start`:
...
// per le applicazioni e la loro barra dei menu rimanere attivo fino a quando l'utente esce
...
Clona ed esegui il codice mostrato in questo tutorial utilizzando il repository [`electron/electron-quick-start`][quick-start].
...
Per una lista degli standard e degli strumenti per iniziare il proprio processo di sviluppo, vedi la [Documentazione per gli Standard e le Command Line Interfaces][boilerplates].
