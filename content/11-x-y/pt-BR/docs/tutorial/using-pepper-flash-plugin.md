...
No macOS e Linux, os detalhes do plugin Pepper Flash podem ser encontrados navegando até `chrome://version` no navegador Chrome. Sua localização e versão são úteis para o suporte do Pepper Flash do Electron. Você também pode copiá-lo para outro local.
...
No Windows, o caminho passado para `--ppapi-flash-path` tem que usar `\` como caminho delimitador, usar caminhos no estilo POSIX não funcionará.
