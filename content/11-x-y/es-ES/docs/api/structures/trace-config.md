...
* `recording_mode` String (optional) - Can be `record-until-full`, `record-continuously`, `record-as-much-as-possible` or `trace-to-console`. Ajustado por defecto a `record-until-full`.
* `trace_buffer_size_in_kb` number (optional) - maximum size of the trace recording buffer in kilobytes. Valor por defecto: 100MB.
* `trace_buffer_size_in_events` number (optional) - maximum size of the trace recording buffer in events.
* `enable_argument_filter` boolean (opcional) - si true, filtra datos de eventos de acuerdo a una lista especifica de eventos que han sido manualmente revisadas para no incluir ningún PII. See [the implementation in Chromium][trace_event_args_whitelist.cc] for specifics.
