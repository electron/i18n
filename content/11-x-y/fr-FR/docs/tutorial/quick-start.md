# Démarrage Rapide

Electron vous permet de créer des applications de bureau avec du pur JavaScript fournissant un runtime avec des API natives riches (système d'exploitation). Vous pourriez le voir comme une variante d'une application Node.js directement exécutable sur le bureau au lieu des serveurs Web.

L'ancien document "Démarrage rapide" qui existait ici a été scindé en deux documents :

* Pour voir comment une application Electron simple est construite, voir [Écrire votre première application Electron][first-app]
* Pour consulter l'architecture des processus, voir [Processus principal et de rendu][processes].

Pour en savoir plus sur Electron, Découvrez les [guides officiels][readme].

[first-app]: ./first-app.md
[processes]: ./application-architecture.md#main-and-renderer-processes
[readme]: ../

