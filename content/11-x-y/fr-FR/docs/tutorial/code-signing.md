...
Si vous utilisez l'outil de compilation préféré d'Electron, faire signer votre application et notarier nécessite quelques ajouts à votre configuration. [Forge](https://electronforge.io) est une collection des outils officiels d'Electron, en utilisant [`electron-packager`][], [`electron-osx-sign`][], et [`electron-notarize`][] sous le capot.
