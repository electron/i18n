...
* `webContents` [WebContents](web-contents.md)
* `details` Object
  * `reason` String - The reason the render process is gone.  Možné hodnoty:
...
* `details` Object
  * `type` String - Process type. One of the following values:
    ...
    * `Unknown`
  * `reason` String - The reason the child process is gone. Možné hodnoty:
    * `clean-exit` - Process exited with an exit code of zero
    * `abnormal-exit` - Process exited with a non-zero exit code
    * `killed` - Process was sent a SIGTERM or otherwise killed externally
    * `crashed` - Process crashed
...
Vrací `Boolean` - `true` pokud Electron dokončil inicializaci, `false` v opačném případě. See also `app.whenReady()`.
...
To work with Electron's `autoUpdater` on Windows, which uses [Squirrel][Squirrel-Windows], you'll want to set the launch path to Update.exe, and pass arguments that specify your application name. Například:
