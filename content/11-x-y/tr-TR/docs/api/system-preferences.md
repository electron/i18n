...
Eğer [DWM composition][dwm-composition] (Aero Glass) aktifse `true` aksi takdirde `false` `Boolean` değerini döndürecektir.
...
RGB onaltılık form (`#ABCDEF`) içindeki sistem renk ayarlarını `String` olarak döndürür. See the [Windows docs][windows-colors] and the [macOS docs][macos-colors] for more details.
