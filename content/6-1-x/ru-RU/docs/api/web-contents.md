...
* `options` Object (optional)
  ...
  * `baseURLForDataURL` String (optional) - Base url (with trailing path separator) for files to be loaded by the data url. Это необходимо, только если указанный `url` это URL данных и необходимо загрузить другие файлы.
...
#### `contents.setIgnoreMenuShortcuts(ignore)` _Экспериментально_
...
* `keyCode` String (**required**) - The character that will be sent as the keyboard event. Используйте только корректные коды ключей в [Accelerator](accelerator.md).
