...
__Note__: If the `main` field is not present in `package.json`, Electron will attempt to load an `index.js` (as Node.js does). Nếu đây thực ra là một ứng dụng Node cơ bản, bạn nên thêm một script `start` để ra lệnh cho `node` thực thi gói hiện hành:
