...
* `event` Event
* `webPreferences` Object - The web preferences that will be used by the guest page. このオブジェクトを変更して、ゲストページの設定を調整できます。
* `params` Object - The other `<webview>` parameters such as the `src` URL. このオブジェクトを変更して、ゲストページのパラメーターを調整できます。
...
関連付けられたウインドウがコンソールメッセージを出力すると発生します。 Will not be emitted for windows with *offscreen rendering* enabled.
...
* `options` Object (任意)
  ...
  * `deviceName` String (任意) - 使用するプリンタデバイスの名前をセットします。 Default is `''`.
...
* `options` Object (任意)
  ...
  * `activate` Boolean (optional) - Whether to bring the opened devtools window to the foreground. 省略値は `true` です。
...
* `keyCode` String (**required**) - The character that will be sent as the keyboard event. [Accelerator](accelerator.md) で有効なキーコードのみを使用する必要があります。
