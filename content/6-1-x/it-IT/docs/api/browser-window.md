...
* `options` Object (optional)
  ...
  * `enableLargerThanScreen` Boolean (optional) - Enable the window to be resized larger than screen. Di default è `false`.
  ...
  * `transparent` Boolean (optional) - Makes the window [transparent](frameless-window.md). Di default è `false`.
...
* `flag` Boolean
* `level` String (opzionale) _macOS_ - I valori includono `normal`, `floating`, `torn-off-menu`, `modal-panel`, `main-menu`, `status`, `pop-up-menu`, `screen-saver`, e ~~`dock`~~ (Deprecato). Il valore predefinito è `floating`. Vedi [Documentazione macOS](https://developer.apple.com/documentation/appkit/nswindow/level) per maggiori dettagli.
