...
If `gclient sync` is interrupted while using the git cache, it will leave the cache locked. Per rimuovere il blocco, passare il valore `--break_repo_lock` a `gclient sync`.
