...
* `opțiuni` Object -Obiect (opțional) 
  ...
  * `zoomToPageWidth` Boolean (opțional) - Controlează comportamentul în macOS când opțiunea- făcând clic pe butonul verde stoplight din bara de instrumente sau făcând clic în meniu pe articolul Window > Zoom. If `true`, the window will grow to the preferred width of the web page when zoomed, `false` will cause it to zoom to the width of the screen. This will also affect the behavior when calling `maximize()` directly. Modul implicit este `false-fals`.
