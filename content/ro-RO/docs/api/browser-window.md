...
* `opțiuni` Object -Obiect (opțional) 
  ...
  * `webPreferences - Preferințele web` Object-Obiect (optional) - Setări ale caracteristicilor paginii web. 
    ...
    * `sandbox` Boolean (opțional) - Dacă set- setează, acest lucru va face ca sandbox-cutie de nisip să redea asociarea cu fereastra, facând-o compatibilă cu Chromium OS-nivel sandbox și dezabilitând motorul Node.js. This is not the same as the `nodeIntegration` option and the APIs available to the preload script are more limited. Read more about the option [here](sandbox-option.md). **Note:** This option is currently experimental and may change or be removed in future Electron releases.
