...
Calling `event.preventDefault()` will prevent Electron from automatically creating a new [`BrowserWindow`](browser-window.md). If you call `event.preventDefault()` and manually create a new [`BrowserWindow`](browser-window.md) then you must set `event.newGuest` to reference the new [`BrowserWindow`](browser-window.md) instance, failing to do so may result in unexpected behavior. Наприклад:
...
#### `contents.setIgnoreMenuShortcuts(ignore)` _Експериментальний_
...
* `options` Object (optional)
  ...
  * `wordStart` Boolean (optional) - Whether to look only at the start of words. за замовчуванням `false`.
...
Increase the capturer count by one. The page is considered visible when its browser window is hidden and the capturer count is non-zero.

This also affects the Page Visibility API.

#### `contents.decrementCapturerCount()`

Decrease the capturer count by one. The page will be set to hidden or occluded state when its browser window is hidden or occluded and the capturer count reaches zero.
...
* `options` Object (optional)
  * `silent` Boolean (optional) - Don't ask user for print settings. За замовчуванням `false`.
  * `printBackground` Boolean (optional) - Prints the background color and image of the web page. За замовчуванням `false`.
  * `deviceName` String (optional) - Set the printer device name to use. Must be the system-defined name and not the 'friendly' name, e.g 'Brother_QL_820NWB' and not 'Brother QL-820NWB'.
  * `color` Boolean (optional) - Set whether the printed web page will be in color or grayscale. За замовчуванням `true`.
  ...
  * `landscape` Boolean (optional) - Whether the web page should be printed in landscape mode. За замовчуванням `false`.
