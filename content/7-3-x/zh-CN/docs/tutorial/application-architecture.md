...
> #### 题外话：进程间通讯
> 
> Electron为主进程（ main process）和渲染器进程（renderer processes）通信提供了多种实现方式，如可以使用[`ipcRenderer`](../api/ipc-renderer.md) 和 [`ipcMain`](../api/ipc-main.md)模块发送消息，使用 [remote](../api/remote.md)模块进行RPC方式通信。 这里也有一个常见问题解答：[web页面间如何共享数据][share-data]。
...
绝大多数的Node.js模块都_不_是原生的， 只有大概400~650个模块是原生的。 当然了，如果你的确需要原生模块，可以在这里查询[如何重新为Electron编译原生模块][native-node](很简单)。
