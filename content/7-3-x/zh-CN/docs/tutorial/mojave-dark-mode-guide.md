...
"Native Interfaces" include the file picker, window border, dialogs, context menus and more; basically anything where the UI comes from macOS and not your app.  The default behavior as of Electron 7.0.0 is to opt in to this automatic theming from the OS.  如果您不想自动切换主题，您必须在 `Info.plist` 文件中将 `NSRequiresAquaSystemAppearance` 设为 `true`。  请注意，一旦Electron开始使用 10.14 SDK 构建，您就不再能禁用主题的切换。
