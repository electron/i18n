...
* `args` String (optional) - `프로그램`이 실행될 때의 명령줄 인수 `type`이 `task`인 경우에만 설정해야합니다.
* `title` String (optional) - 점프 목록의 항목에 대해 표시할 텍스트입니다. `type`이 `task`인 경우에만 설정해야합니다.
* `description` String (optional) - 작업에 대한 설명 (도구 설명에 표시). `type`이 `task`인 경우에만 설정해야합니다.
