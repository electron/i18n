# Các Chrome Command Line Switch được hỗ trợ
...
Gives the per-module maximal V-logging levels to override the value given by `--v`. Ví dụ. `my_module=2,foo*=3` would change the logging level for all code in source files `my_module.*` and `foo*.*`.

Any pattern containing a forward or backward slash will be tested against the whole pathname and not only the module. Ví dụ. `*/foo/bar/*=2` would change the logging level for all code in the source files under a `foo/bar` directory.
