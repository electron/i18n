...
￼` ￼ ￼Note that `webkitdirectory` no longer exposes the path to the selected folder. ￼Если вам нужен путь к выбранной папке, а не к содержимому папки, ￼смотрите `dialog.showOpenDialog` API ([Ссылка](https://github.com/electron/electron/blob/master/docs/api/dialog.md#dialogshowopendialogbrowserwindow-options)).
