...
Se la tua app è impacchettata con [`electron-costruttore`][electron-builder-lib] puoi usare il modulo [electron-updater][], che non richiede un server e consente aggiornamenti da S3, GitHub o qualsiasi altro host di file statico. Questo meccanismo di aggiornamento incorporato di Electron è evitato, il resto di questa documentazione non si applicherà all'updater di `electron-costruttore`.
