...
app. enableSandbox ()
app. whenReady (). then (() => {
  //no es necesario pasar ' Sandbox: true ' desde que se llamó a app. enableSandbox () '.
  Win = New BrowserWindow ()
  Win. loadURL (' http://Google.com ')
})
```
...
app. whenReady (). then (() => {
  Win = New BrowserWindow ({
    webPreferences: {
      Sandbox: true,
      precarga: path. join (App. getAppPath (), ' preload. js ')
    }
  })
  Win. loadURL (' http://Google.com ')
})
...
// ámbito privado que puede acceder a un subconjunto de APIs de rendererizado de Electron. Sin
//Contextisolación habilitada, es posible filtrar por accidente
//Globals como ipcRenderer a contenido Web.
