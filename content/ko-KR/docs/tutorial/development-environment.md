...
일단 설치되면, 모든 것이 예상대로 작동하는지 확인하십시오. 시작메뉴를 열고 PowerShell(또는 명령 프롬프트) 이라고 입력하여 Windows PowerShell(또는 명령 프롬프트)을 찾습니다. `PowerShell`또는 다른 명령 행 클라이언트를 열면 `node`및`npm` 모두 사용할 수 있습니다.
