...
Electron은 기본적으로 독점 코덱을 포함하는 `ffmpeg`의 버전을 제공합니다. 이 코덱이없는 버전은 각 릴리스와 함께 빌드되어 배포됩니다. Each Chrome upgrade should verify that switching this version is still supported.
