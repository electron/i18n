...
Si votre application fonctionne avec Electron 6.0.8 ou supérieur, vous pouvez maintenant la compiler pour Windows 10 sur Arm. Cela améliore considérablement les performances, mais nécessite la recompilation de tous les modules natifs utilisés dans votre application. Il peut également exiger de petits correctifs à votre script de construction et d’emballage.
...
Beaucoup de code spécifique à Windows contient si ... d’autre logique qui sélectionne entre les architectures x64 ou x86.
...
Par défaut, `node-gyp` déballe les en-têtes de nœuds d’Electron et télécharge les versions x86 et x64 de `node.lib` en `%APPDATA%\..\Local\node-gyp\Cache`, mais il ne télécharge pas la version arm64 ([un correctif pour cela est en développement](https://github.com/nodejs/node-gyp/pull/1875).) Pour corriger ceci:
