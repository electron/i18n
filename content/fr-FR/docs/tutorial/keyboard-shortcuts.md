...
Les raccourcis clavier locaux ne sont déclenchés que lorsque l'application a le focus. Pour configurer un raccourci clavier local, vous devez spécifier une propriété [`accelerator`][] lors de la création d’un</a> MenuItemdans le module [Menu][] .</p> 
...
menu.append(new MenuItem({
  ...
  submenu: [{
    ...
    clic: () => { console.log ('Electron rocks!') }
  }]
}))
...
app.whenReady().then()) => {
  globalShortcut.register('Alt+CommandOrControl+I', () => { console
    .log ('Electron aime les raccourcis mondiaux!')
  })
}).then (createWindow)
```
...
Mousetrap.bind('?', () => { console.log ('show raccourcis!') })
...
Mousetrap.bind(['command+k', 'ctrl+k'], () => { console
  .log ('commande k ou contrôle k')

  // retour faux pour empêcher le comportement par défaut et arrêter l’événement de bouillonner
  retour faux
}) séquences de style
