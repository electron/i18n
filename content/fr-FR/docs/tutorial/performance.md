...
![Profil de performance de CPU](../images/performance-cpu-prof.png)

![Profil de performance de la mémoire du tas](../images/performance-heap-prof.png)
...
3) Avoid using blocking I/O operations in the main process. Bref, chaque fois que des modules coeur de Node.js (comme `fs` ou `child_process`) offrent une version synchrone ou une version asynchrone vous devriez préférer la variante asynchrone et non-bloquant .
