...
Electron possède des APIs pour configurer l'icône de l'application dans la barre des tâches de Windows. Cette API prend en charge à la fois les fonctionnalités windows uniquement [comme la création d’un `JumpList`](#jumplist), [vignettes personnalisées et barres d’outils](#thumbnail-toolbars), [superpositions icône](#icon-overlays-in-taskbar), et le soi-disant [« Flash Frame » effet](#flash-frame), et les fonctionnalités multiplateformes comme [documents récents][recent-documents] et [application progressent][progress-bar].

## JumpList 

Windows permet aux applications de définir un menu contextuel personnalisé s'affichant lorsque les utilisateurs effectuent un clic-droit sur l’icône de l’application dans la barre des tâches. Ce menu contextuel est appelé `JumpList`. Vous spécifiez les actions personnalisées dans `Tasks` catégorie de JumpList, citées dans [MSDN][msdn-jumplist]:
...
Pour définir les tâches utilisateur de votre application, vous pouvez utiliser [app.setUserTasks][setusertaskstasks] API.
...
Tel que cité [MSDN][msdn-thumbnail]:
...
Pour définir la barre d’outils miniature dans votre application, vous devez utiliser [BrowserWindow.setThumbarButtons][setthumbarbuttons]
...
win.setThumbarButtons([
  {
    tooltip: 'button1',
    icône: path.join(__dirname, 'button1.png'),
    click () { console.log ('button1 clicked') }
  }, {
    tooltip: 'button2',
    icône: path.join(__dirname, 'button2.png'),
...
Tel que cité [MSDN][msdn-icon-overlay]:
...
Pour définir l’icône de superposition pour une fenêtre, vous devez utiliser l' [BrowserWindow.setOverlayIcon][setoverlayicon] API.
...
Tel que cité [MSDN][msdn-flash-frame]:
...
Pour flasher le bouton de barre des tâches BrowserWindow, vous devez utiliser [BrowserWindow.flashFrame][flashframe] API.
