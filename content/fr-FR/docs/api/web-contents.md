...
* `handler` fonction<{action: 'deny'} | {action: 'allow', overrideBrowserWindowOptions?: BrowserWindowConstructorOptions}>
  * `details` objet
    ...
    * `traits`Fiche-Une liste de traits de fenetre separee par comas, a ete fournie a`window.open()`. Va donner `{action: 'deny'} | {action: 'permettre', surpasserOptionsduNavigateurFenetreBrowser?: OptionsConstructeurNavigateursFenetre}`- - `refuser` va supprimer la creation dans la fenetre nouvelle. `permettre`va permettre la creation d'une fenetre nouvelle. Spécifier `overrideBrowserWindowOptions` permet la personnalisation de la fenêtre créée. Le retour d’une valeur non reconnue telle qu’un nul, non défini ou un objet sans valeur d’action reconnue entraînera une erreur de console et aura le même effet que le retour de `{action: 'deny'}`.
