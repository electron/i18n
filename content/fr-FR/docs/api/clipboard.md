...
Sur Linux, il y a aussi un `selection` presse-papiers. Pour le manipuler vous devez passer `selection` à chaque méthode :
...
**note : les** expérimentales sont marquées comme telles et pourraient être supprimées à l’avenir.

### `clipboard.readText([type])`

* `type` String (facultatif) - Peut être `selection` ou `clipboard`; par défaut est « presse-papiers ». `selection` n’est disponible que sur Linux.
...
const texte = clipboard.readText ()
console.log (texte)
...
**Note:** La plupart des applications sur Windows ne prend pas en charge le pâturage des signets en eux afin vous pouvez utiliser des `clipboard.write` pour écrire à la fois un signet et un texte de repli sur le presse-papiers .
...
Cette méthode utilise un IPC synchronisé lorsqu'il est appelé depuis le processus de rendu. La valeur mise en cache est relue à partir du panneau de lecture chaque fois que l’application est activée.
...
### `clipboard.writeBuffer(format, buffer[, type])` _'_
