...
#### `contents.setIgnoreMenuShortcuts(ignore)` _Đang thử nghiệm_
...
* `options` Object (optional)
  ...
  * `printBackground` Boolean (optional) - Prints the background color and image of the web page. Mặc định là `false`.
  ...
  * `landscape` Boolean (optional) - Whether the web page should be printed in landscape mode. Mặc định là `false`.
...
If you want to get the `frameId` of a given renderer context you should use the `webFrame.routingId` value.  Ví dụ.
