...
Emitted when `remote.require()` is called in the renderer process. `event.preventDefault()`를 실행하면 모듈이 반환되지 않습니다. `event.returnValue`를 설정하여 임의의 값을 반환할 수 있습니다.
...
Emitted when `remote.getBuiltin()` is called in the renderer process. `event.preventDefault()`를 실행하면 모듈이 반환되지 않습니다. `event.returnValue`를 설정하여 임의의 값을 반환할 수 있습니다.
...
* `options` Object (optional)
  * `silent` Boolean (optional) - Don't ask user for print settings. 기본값은 `false`이다.
  * `printBackground` Boolean (optional) - Prints the background color and image of the web page. 기본값은 `false`이다.
  * `deviceName` String (optional) - Set the printer device name to use. Must be the system-defined name and not the 'friendly' name, e.g 'Brother_QL_820NWB' and not 'Brother QL-820NWB'.
  * `color` Boolean (optional) - Set whether the printed web page will be in color or grayscale. 기본값은 `true`이다.
  ...
  * `landscape` Boolean (optional) - Whether the web page should be printed in landscape mode. 기본값은 `false`이다.
...
If you want to get the `frameId` of a given renderer context you should use the `webFrame.routingId` value.  예:
...
#### `contents.session` _읽기전용_

A [`Session`](session.md) used by this webContents.

#### `contents.hostWebContents` _읽기전용_
