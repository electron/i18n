...
Это заставит окно поддерживать соотношение сторон. The extra size allows a developer to have space, specified in pixels, not included within the aspect ratio calculations. Этот метод уже учитывает разницу между размером окна и размером его содержимого.

Рассмотрим нормально окно в HD видео-плеером, и связанными с ним контроллерами. Perhaps there are 15 pixels of controls on the left edge, 25 pixels of controls on the right edge and 50 pixels of controls below the player. Для сохранения соотношения сторон 16:9 (стандартное соотношение сторон для HD @1920x1080) в плеере, мы можем вызвать эту функцию с аргументами 16/9 и [ 40, 50 ]. Второй аргумент не заботится о том, где дополнительная ширина и высота находятся внутри содержимого вида, в котором они существуют. Sum any extra width and height areas you have within the overall content view.
