...
4. [Sử dụng `ses.setPermissionRequestHandler()` trong tất cả các session tải nội dung từ xa](#handle-session-permission-requests-from-remote-content)
...
11. [WebViews: Không sử dụng `allowpopups`](#do-not-use-allowpopups)
