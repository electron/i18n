...
* `args` String (facultatif) - Les arguments de ligne de commande lorsque `program` est exécuté. Doit uniquement être défini si `type` est `task`.
* `title` String (facultatif) - Le texte à afficher pour l'élément dans la Jump List. Doit uniquement être défini si `type` est `task`.
* `description` String (facultatif) - Description de la tâche (affichée dans une infobulle). Doit uniquement être défini si `type` est `task`.
