...
應用程式的易用性很重要，所以我們在 [Devtron][devtron] 及 [Spectron][spectron] 中加入了新功能，讓開發人員更容易打造出適用每個人的應用程式。
...
你可以在 [Spectron 說明文件][spectron-a11y]中看到這個功能的更多資訊。
...
上述兩組工具都是用 Google 為了 Chrome 打造的 [Accessibility Developer Tools][a11y-devtools] 程式庫。 你可以在這個函式庫的[儲存庫 Wiki][a11y-devtools-wiki] 中找到易用性稽查規則的詳細資訊。
...
透過 [`app.setAccessibilitySupportEnabled(enabled)`][setAccessibilitySupportEnabled]，你可以把協助工具的開關放在應用程式喜好設定中，讓使用者自行決定。 使用者系統上的輔助工具優先權更高，會蓋過這項設定值。

### 輔具

Electron 應用程式偵測到輔助技術 (Windows) 或 VoiceOver (macOS) 時，會自動開啟協助工具。 細節可參考 Chrome 的 [協助工具文件][a11y-docs]。
