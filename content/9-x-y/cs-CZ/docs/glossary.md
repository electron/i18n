...
IPC značí komunikaci mezi procesy. Electron uses IPC to send serialized JSON messages between the [main][] and [renderer][] processes.
...
Akronym pro Mac App Store společnosti Apple. For details on submitting your app to the MAS, see the [Mac App Store Submission Guide][].
...
The renderer process is a browser window in your app. Unlike the main process, there can be multiple of these and each is run in a separate process. Lze je také skrýt.
