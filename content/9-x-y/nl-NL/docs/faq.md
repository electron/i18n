...
However if you are using the built-in module but still getting this error, it is very likely you are using the module in the wrong process. Bijvoorbeeld `electron.app` kan alleen worden gebruikt in het hoofdproces, terwijl `electron.webFrame` alleen beschikbaar is in renderer processen.
