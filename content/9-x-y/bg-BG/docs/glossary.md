...
ASAR е абревиатура от Atom Shell Archive Format или ядрен формат за архивиране. [asar][asar] е прост, подобен на `tar` формат за архиви, който съдържа файлове в един единствен файл. Електрон може да чете произволни файлове от него без разопаковане на целия файл.
...
Диск изображение за Apple е опаковачен формат, използван от macOS. DMG файловете са често използвани за разпространение на "инсталаторни" приложения. [електрон-builder][] поддържа `dmg` като изграждаща цел.
...
Споделена Библиотека, която включва [Chromium Content module][] и всички негови зависимости (например, Blink, [V8][], и т.н.). Наричана също "libcc".

- [github.com/electron/libchromiumcontent](https://github.com/electron/libchromiumcontent)

### основен процес

The main process, commonly a file named `main.js`, is the entry point to every Electron app. It controls the life of the app, from open to close. Той също така управлява родни за устройството елементи като меню, лента с менюта, док, табла/Tray и др. The main process is responsible for creating each new renderer process in the app. The full Node API is built in.
...
Native modules (also called [addons][] in Node.js) are modules written in C or C++ that can be loaded into Node.js or Electron using the require() function, and used as if they were an ordinary Node.js module. Те се използват главно за осигурява интерфейс между JavaScript код в Node.js и C/C ++ библиотеки.
...
Процесът е инстанция на компютърна програма, която се изпълнява. Електрон приложенията, които използват [main][] и един или много [renderer][] процеси всъщност изпълняват няколко програми едновременно.
...
Squirrel е системна рамка с отворен код, която позволява на Електрон приложенията да се обновяват автоматично, когато има издадени нови версии. Виж [autoUpdater][] API за повече информация как да започнем работа с Squirrel.
...
`webview` tags are used to embed 'guest' content (such as external web pages) in your Electron app. They are similar to `iframe`s, but differ in that each webview runs in a separate process. Не разполага със същите разрешения като вашата уеб страница и всички взаимодействия между приложението и вградено съдържание ще бъде асинхронно. Това поддържа вашето приложение в безопасност от вградените съдържания.

[addons]: https://nodejs.org/api/addons.html
[asar]: https://github.com/electron/asar
[autoUpdater]: api/auto-updater.md
[Chromium Content module]: https://www.chromium.org/developers/content-module
[електрон-builder]: https://github.com/electron-userland/electron-builder
[electron-builder]: https://github.com/electron-userland/electron-builder
[Mac App Store Submission Guide]: tutorial/mac-app-store-submission-guide.md
[main]: #main-process
[renderer]: #renderer-process
[Използват родния Node модули]: tutorial/using-native-node-modules.md
[V8]: #v8
