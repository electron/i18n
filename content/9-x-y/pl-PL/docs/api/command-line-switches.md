...
[App.commandLine.appendSwitch][append-switch] można użyć, aby dołączyć je w skrypcie głównym aplikacji, zanim zostaną wyemitowane zdarzenie [ready][ready] modułu [aplikacji][app]:
...
Flagi związane z debugowaniem, zobacz więcej w poradniku dla debugowania [Main Process][debugging-main-process].
...
Gives the per-module maximal V-logging levels to override the value given by `--v`. Np. `my_module=2,foo*=3` would change the logging level for all code in source files `my_module.*` and `foo*.*`.

Any pattern containing a forward or backward slash will be tested against the whole pathname and not only the module. Np. `*/foo/bar/*=2` would change the logging level for all code in the source files under a `foo/bar` directory.
