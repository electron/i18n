...
Когда `callback` вызывается без значения, с числом или объектом, имеющим свойство `error`, запрос `request` завершится ошибкой `error` с номером, который вы указали. For the available error numbers you can use, please see the [net error list][net-error].
