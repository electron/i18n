...
你也可以使用 [app.clearRecentDocuments][clearrecentdocuments] API 来清空最近文件列表。
...
为了在 Windows 上使用这个特性，你的应用需要被注册为这类文件的处理程序，否则，在你注册之前，文件是不会出现在跳转列表里的。 你可以在 [Application Registration][app-registration] 里找到所有关于注册事宜的说明。
