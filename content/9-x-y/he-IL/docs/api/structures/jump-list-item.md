...
* `args` - מחרוזת (אופציונלי) - הארגומנטים של שורת פקודה כאשר `program` מבוצעת. צריך להיות מוגדר אך ורק אם `type` הוא `task`.
* `title` מחרוזת (אופציונלית) - הטקסט שיש להציג לפריט בJumpList. צריך להיות מוגדר אך ורק אם `type` הוא `task`.
* `description` מחרוזת (אופציונלי) - התיאור של המשימה (מוצג בtooltip). צריך להיות מוגדר אך ורק אם `type` הוא `task`.
...
* `workingDirectory` - מחרוזת (אופציונלי) - הWorking Directory. ריק כברירת מחדל.
