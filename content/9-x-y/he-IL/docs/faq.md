...
However if you are using the built-in module but still getting this error, it is very likely you are using the module in the wrong process. לדוגמה, `electron.app` יכול להיות בשימוש אך ורק בתהליך הראשי, בעוד `electron.webFrame` נגיש רק בתהליכים המגישים.
