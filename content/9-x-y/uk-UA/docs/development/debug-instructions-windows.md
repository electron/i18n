...
* **Visual Studio з інструментами C++**: Безкоштовні зміни спільноти Visual Studio 2013 та Visual Studio 2015 працюють. Once installed, [configure Visual Studio to use GitHub's Electron Symbol server](setting-up-symbol-server.md). It will enable Visual Studio to gain a better understanding of what happens inside Electron, making it easier to present variables in a human-readable format.

* **ProcMon**: [безкоштовний інструмент SysInternals][sys-internals], що дозволяє вам переглядати параметри процесів, дескриптори файлів та операції над реєстром.
...
В якості введення в базові та розширені можливості налагодження ProcMon, перейдіть до [цієї відео-інструкції][procmon-instructions] від Microsoft.
