...
> Membuat sebuah peramban dengan perender yang dapat beroperasi di dalam OS sandbox Chromium. With this option enabled, the renderer must communicate via IPC to the main process in order to access node APIs. However, in order to enable the Chromium OS sandbox, electron must be run with the `--enable-sandbox` command line argument.
