...
* `pagpipilian` Bagay 
  ...
  * `submenu` (MenuItemConstructorOptions[] | Menu) (optional) - Should be specified for `submenu` type menu items. Kung ang `submenu` ay tinukoy na, ang `type: 'submenu'` ay maaaring tanggalin. If the value is not a `Menu` then it will be automatically converted to one using `Menu.buildFromTemplate`.
