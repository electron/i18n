...
## Planificat Breaking Modificări API (10.0)
...
## Modificări planificate ale API-ului (9.0)
...
const getGuestForWebContent = (webContentsId, Conținut) => {
  ...
  dacă (vizitator. conţinut gazdă! = conținuturi) {
    aruncați o nouă Error(`Access negat la webContents`)
  }
  return guest
}
...
### API modificat: `powerMonitor.querySystemIdleTime` este acum `powerMonitor.getSystemIdleState`
