...
1) Usando [`forgia-electron`][electron-forge] o [`builder-electron`][electron-builder], entrambi strumenti con supporto `snap` fuori dalla scatola. Questa è l'opzione più facile. 2) Usando `electron-istallatore-snap`, che prende l'output dell'`electron-impacchettatore`. 3) Usando un pacchetto `.deb` preesistente.
...
Mentre _è possibile_ installare `snapcraft` su macOS usando Homebrew, non è possibile costruire pacchetti `snap` ed è focalizzato su pacchetti gestionali nel negozio.

## Usando `electron-installatore-snap`

Il modulo fuziona come [`electron-winsallatore`][electron-winstaller] e simili moduli di questo, con scopi limitati alla costruzione dei pacchetti snap. Puoi installarlo con:
...
Se hai una pipeline di costruzione esistente, puoi usare `electron-installatore-snap` programmaticamente. Per altre informazioni, vedi i [documenti API Snapcraft][snapcraft-syntax].
...
Se non hai già un pacchetto `.deb`, usando `electron-installatore-snap` potrebbe essere un percorso più facile per creare pacchetti snap. Comunque, esistono soluzioni multiple per creare pacchetti Debian, inclusi [`electron-forgia`][electron-forge], [`electron-costruttore`][electron-builder] o [`electron-installatore-debian`][electron-installer-debian].
