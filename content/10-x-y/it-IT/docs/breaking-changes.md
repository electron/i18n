...
### Tipi di cambiamenti di rottura

Questo documento utilizza la seguente convenzione per categorizzare le modifiche di rottura:
...
- **Comportamento cambiato:** Il comportamento di Electron è cambiato, ma non in modo tale che un'eccezione sarà necessariamente lanciata.
- **Default Changed:** Code depending on the old default may break, not necessarily throwing an exception. Il vecchio comportamento può essere ripristinato specificando esplicitamente il valore.
- **Deprecato:** Un'API è stata contrassegnata come deprecata. L'API continuerà a funzionare, ma emetterà un avviso di deprecazione, e sarà rimosso in una versione futura.
- **Rimosso:** Un'API o una funzionalità è stata rimossa, e non è più supportata da Electron.
...
### API modificata: `powerMonitor.querySystemIdleTime` ora è `powerMonitor.getSystemIdleState`
...
Per esempio Re-enabling the webviewTag
