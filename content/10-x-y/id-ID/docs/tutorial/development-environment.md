...
Pertama, pasang Node.js versi terbaru. We recommend that you install either the latest `LTS` or `Current` version available. Kunjungi [halaman unduh Node.js][node-download] dan pilih `Windows Installer`. Once downloaded, execute the installer and let the installation wizard guide you through the installation.
