...
Zespół Electron utrzymuje [update.electronjs.org][], darmową i otwarto-źródłową usługę, której aplikacje Electron mogą używać do samodzielnej aktualizacji. Usługa jest zaprojektowana dla aplikacji Electrona, które spełniają następujące kryteria:
...
Najprostszym sposobem na korzystanie z tej usługi jest zainstalowanie [update-electron-app][], modułu Node.js wstępnie skonfigurowanego do użytku z update.electronjs.org.
...
Jeśli potrzebujesz dostosować konfigurację, możesz [przekazać opcje `update-electron-app`][update-electron-app] lub [użyć usługi aktualizacji bezpośrednio][update.electronjs.org].
...
- [Hazel][hazel] - Aktualizuj serwer dla prywatnych lub otwartych aplikacji, które można wdrożyć za darmo na [Teraz][now]. Wyciąga on z [GitHub Releases][gh-releases] i zwiększa moc CDN GitHuba.
- [Nuts][nuts] – używa również [wersji GitHub][gh-releases], ale pamiętaj o aktualizacjach aplikacji na dysku i obsługuje prywatne repozytoria.
- [electron-release-server][electron-release-server] – zapewnia kokpit menedżerski do obsługi wydań i nie wymaga wydania na GitHubie.
- [Nucleus][nucleus] – kompletny serwer aktualizacji aplikacji Electron obsługiwany przez Atlassian. Obsługuje wiele aplikacji i kanałów; używa statycznego sklepu plików , aby zminimalizować koszty serwera.
...
## Ręcznie obsługiwane aktualizacje
