...
렌더링의 두 가지 모드를 사용할 수 있으며 더 효율적이도록 `'paint'` 이벤트에 더티 영역만 전달됩니다. 렌더링을 중지하고 계속하며 프레임률을 설정할 수 있습니다. 지정된 프레임 레이트는 상한값이며, 웹 페이지에서 아무 일도 일어나지 않으면 프레임이 생성되지 않습니다. 최대 프레임 레이트는 240입니다. 그 이유는 그 이상의 경우에는 장점이 없고 성능만 손실되기 때문입니다.
