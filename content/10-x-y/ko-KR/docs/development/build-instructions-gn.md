...
Chromium과 Chromium 관련 의존성을 가져오는데 사용되는 툴셋인 [`depot_tools`][depot-tools]를 설치해야 합니다.
...
반복된 빌드 작업 속도를 향상시키기 위해 [sccache][sccache]를 사용할 수 있습니다. Add the GN arg `cc_wrapper = "sccache"` by running `gn args out/Testing` to bring up an editor and adding a line to the end of the file.
