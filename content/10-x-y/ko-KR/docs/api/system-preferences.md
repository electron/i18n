...
**참고:** 이 API가 macOS 10.15 Catalina에서 "자동" 다크 모드에 있을 때 정확한 값을 반환하게 하려면 `Info.plist` 파일에 `NSRequiresAquaSystemAppearance=false`를 추가하거나 Electron 버전이 `7.0.0 이상`이어야 합니다.  See the [dark mode guide](../tutorial/mojave-dark-mode-guide.md) for more information.
...
**참고:** 이 API는 macOS Sierra 10.12.2 버전 이하에선 `false`를 반환합니다.
...
**참고:** 이 API는 macOS Sierra 10.12.2 버전 이하에선 거부된 Promise를 반환합니다.
