...
* `options` Object (opzionale) 
  ...
  * `fullscreenWindowTitle` Boolean (opzionale) - Mostra il titolo nella barra del titolo in modalità a schermo intero su macOS per tutte le opzioni di ` titleBarStyle </ 0>.
Il valore predefinito è <code> false</ 0>.</li>
<li><code>thickFrame` Boolean (optional) - Use `WS_THICKFRAME` style for frameless windows on Windows, which adds standard window frame. Setting it to `false` will remove window shadow and window animations. Di default `true`.
