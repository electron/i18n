...
В Electron Node.js и Chromium используют один экземпляр V8 - обычно это версия, которую использует Chromium. В большинстве случаев это *работает*, но иногда требует обновления Node.js.
...
Например, Electron использует только библиотеку рендеринга от Chromium, а не весь Chromium целиком. Это упрощает обновление Chromium, но также означает, что некоторые функции браузера, найденные в Google Chrome, не существуют в Electron.
