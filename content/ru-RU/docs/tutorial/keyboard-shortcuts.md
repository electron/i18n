...
Событие [` before-input-event `](../api/web-contents.md#event-before-input-event) генерируется перед отправкой событий ` keydown ` и ` keyup ` на странице. It can be used to catch and handle custom shortcuts that are not visible in the menu.
