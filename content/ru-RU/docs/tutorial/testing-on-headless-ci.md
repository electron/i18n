...
Затем создайте виртуальный экран Xvfb и экспортируйте переменную окружения, называемую DISPLAY, которая указывает на него. Chromium in Electron will automatically look for `$DISPLAY`, so no further configuration of your app is required. Этот шаг можно автоматизировать с помощью команды Пола Беттса [xvfb-maybe](https://github.com/paulcbetts/xvfb-maybe): Предварите ваши тестовые команды, используя `xvfb-maybe` и этот маленький инструмент автоматически настроит Xvfb, если требуется текущей системой. On Windows or macOS, it will do nothing.
