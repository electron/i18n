...
* Visual Studio 2017 15.7.2 или выше - [скачать VS 2019 Community Edition для бесплатных](https://www.visualstudio.com/vs/)
  ...
  * Если ваша Visual Studio установлена в каталог, отличающийся от стандартного, вам нужно установить несколько переменных среды, чтобы указать инструментам на путь установки.
    ...
    * `WINDOWSSDKDIR = DRIVE:\path\to\Windows Kits\10`, `DRIVE:` с приводом, что комплекты Windows находится на. Обычно, это `C:`.
...
* Инструменты отладки для Windows Windows SDK 10.0.15063.468, если вы планируете на создание полного распространения с `symstore.exe` используется для создания магазина символов из `.pdb` файлов.
  ...
  * Также необходимо установить инструменты отладки SDK. Если Windows 10 SDK была установлена через visual Studio установщик, то они могут быть установлены, по телефону: `Control Panel` → `Programs` → `Programs and Features` → Выберите "Windows Software Development Kit" → `Change` → `Change` → Проверить "Debugging Tools для Windows" → `Change`. Или вы можете скачать автономный установщик SDK и использовать его для установки инструментов debugging.
...
## Исключить дерево источника из безопасности Windows

Windows Security не нравится один из файлов в исходный код Chromium (см. https://crbug.com/441184), поэтому он будет постоянно удалять его, вызывая `gclient sync` проблемы. Вы можете исключить дерево-источник из мониторинга Windows Security, [этим инструкциям](https://support.microsoft.com/en-us/windows/add-an-exclusion-to-windows-security-811816c0-4dfd-af4a-47e4-c301afe13b26).
...
Чтобы построить для 32bit цели, вы должны пройти `target_cpu = "x86"` как GN arg. Вы можете построить 32bit цель наряду с 64bit цели с помощью различных выходных каталогов для GN, например. `out/Release-x86`, с разными аргументами.
...
Это может произойти во время сборки, когда инструменты отладки для Windows были установлены с набором драйверов Windows. Удалите набор драйверов Windows и установите инструменты отладки с описанными выше шагами.
