...
IPC означает межпроцессное взаимодействие. Electron использует IPC для отправки сообщений JSON между основными [и][] [процессами][] JSON.
...
Основной процесс, обычно файл под названием `main.js`, является точкой входа в каждое Electron. Он контролирует жизнь приложения, от открытых до близких. Также управляет нативными элементами, такими как Menu, Menu Bar, Dock, Tray, и др. Основной процесс отвечает за создание каждого нового процесса рендерера в приложении. Встроенный полный API узла.
...
Acronym для Apple App Store. Подробную информацию о представлении приложения в MAS можно найти [Руководстве по представлению Mac App Store][].
...
Процесс рендерера является окном браузера в вашем приложении. В отличие от основного процесса, может быть несколько из них, и каждый из них работает в отдельном процессе. Они также могут быть скрыты.
...
`webview` теги используются для встраивания содержимого «гостей» (например, внешних веб-страниц) приложении Electron. Они похожи на `iframe`, но отличаются тем, что каждый веб-вид работает в отдельном процессе. Оно не имеет тех же разрешений, так как ваша веб страница и все взаимодействие между вашим приложением и встроенным контентом будут асинхронными. Это позволяет защитить ваше приложение от встраиваемого содержимого.
...
[Руководстве по представлению Mac App Store]: tutorial/mac-app-store-submission-guide.md
[и]: #main-process
[главный(main)]: #main-process
[процессами]: #renderer-process
