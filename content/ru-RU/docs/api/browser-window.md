...
* `options` Object (опционально) 
  ...
  * `webPreferences` Object (опционально) - настройки веб-страниц. 
    ...
    * `nodeIntegrationInSubFrames` Boolean (опционально) - экспериментальная опция для включения поддержки NodeJS в подфреймах, таких как iframes. Все Ваши предзагрузки будут загружены для каждого iframe, Вы можете использовать `process.isMainFrame`, чтобы определить в главном фрейме Вы или нет.
    ...
    * `contextIsolation` Boolean (опционально) - запускать или нет API Electron и определенный `предзагруженный` скрипт в отдельном JavaScript-контексте. По умолчанию `false`. Контекст, в котором `предзагруженный` скрипт будет запускаться, имеет полный доступ к глобальным объектам `document` и `window`, но будет использовать собственный набор JavaScript-типов (`Array`, `Object`, `JSON`, и т.д.) и будет изолирован от любых изменений, сделанных в глобальной среде, загруженной страницы. API Electron будет доступен только в `предзагруженном` скрипте и не будет доступен загруженной странице. Эта опция должна быть использована, когда загружается потенциально ненадежный удаленный контент, чтобы убедиться, что загруженный контент не сможет изменить `предзагруженный` скрипт и любой API Electron, который используется. Эта опция использует ту же самую технику, что используется в [содержимое скриптов Chrome](https://developer.chrome.com/extensions/content_scripts#execution-environment). Вы можете получить доступ к этому контексту в инструментах разработчика, при выборе пункта 'Изолированный Контекст Elctron' в списке на верхней части консольной вкладки.
    ...
    * `webviewTag` Boolean (опционально) - включает [`<webview>`-тег](webview-tag.md). По умолчанию `false`. **Примечание:** `Предзагруженный` скрипт, настроенный для `<webview>`, будет иметь интеграцию NodeJS, когда будет запущен, так что Вы должны убедиться, что удаленный/непроверенный контент не может создавать тег `<webview>` с возможно вредоносным `предзагруженным` скриптом. You can use the `will-attach-webview` event on [webContents](web-contents.md) to strip away the `preload` script and to validate or alter the `<webview>`'s initial settings.
