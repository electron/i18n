...
* `options` Object (необязательно) 
  ...
  * `titleBarStyle` String (необязательно) - Стиль полосы заголовка окна. По умолчанию - `default`. Возможные значения: 
    ...
    * `customButtonsOnHover` Boolean (необязательно) - Отобразить настраиваемые кнопки закрыть, уменьшить и во весь экран на бескаркасных окнах в macOS. These buttons will not display unless hovered over in the top left of the window. These custom buttons prevent issues with mouse events that occur with the standard window toolbar buttons. **Note:** This option is currently experimental.
