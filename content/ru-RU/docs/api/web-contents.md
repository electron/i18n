...
* `handler` Function<{action: 'deny'} | {action: 'allow', overrideBrowserWindowOptions?: BrowserWindowConstructorOptions}>
  * `details` объект
    * `url` String - _решена_ версия URL перешла к `window.open()`. например, открытие окна с помощью `window.open('foo')` -то вроде `https://the-origin/the/current/path/foo`.
    * `frameName` Строка - Название окна, предоставленное в `window.open()`
    * `features` String - Запятая разделен список оконных функций, предоставляемых `window.open()`. Возвращает `{action: 'deny'} | {action: 'allow', overrideBrowserWindowOptions?: BrowserWindowConstructorOptions}` - `deny` отменяет создание нового окна окна. `allow` позволит создать новое окно. Определение `overrideBrowserWindowOptions` позволяет настроить созданное окно. Возвращение непризнанного значения, такого как нулевой, неопределенный или объект без признанного значения действия, приведет к ошибке консоли и будет иметь тот же эффект, что и `{action: 'deny'}`.
