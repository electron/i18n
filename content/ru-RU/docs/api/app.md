...
Например. Этот метод возвращается `true` если ваш процесс является основным экземпляром вашего приложения и ваше приложение должно продолжать загрузку.  Возвращает `false`, если Ваш процесс должен немедленно завершиться, так как он отправил свои параметры другому экземпляру, которые уже приобрел блокировку.
...
Выпускает все блокировки, которые были созданы `requestSingleInstanceLock`. Это позволит нескольким экземплярам приложения снова работать бок о бок.
...
Создает `NSUserActivity` и задает её в качестве текущей активности. Действие, , имеет право [handoff][handoff] другому устройству после этого.
...
По умолчанию Chromium отключает 3D API (например, WebGL) до перезагрузки на основе домена, если процессы GPU происходят слишком часто. Эта функция отключает это поведение.
...
Устанавливает встречный значок для текущего приложения. Установка счета на `0` скроет значок.

На macOS отображается на иконке в Dock. На Linux работает только для лаунчера Unity.

**Примечание:** Unity требует наличия файла `.desktop` для работы, для получения дополнительной информации, пожалуйста, [об интеграции среды рабочего стола][unity-requirement].
...
Для работы с Electron's `autoUpdater` на Windows, которая использует [Squirrel][Squirrel-Windows], вы хотите установить путь запуска для обновления.exe и передать аргументы, которые определяют название приложения. Например:
...
**Note:** рендеринга доступности может существенно повлиять на производительность вашего приложения. Он не должен быть включен по умолчанию.

### `app.showAboutPanel()`

Показать приложение о панели вариантов. Эти параметры могут быть переопределены с `app.setAboutPanelOptions(options)`.
...
Установите описание панели опций. Это переопределит значения, определенные в файле приложения `.plist` macOS. Смотрите [Apple docs][about-panel-options] для получения более подробной информации. На Linux необходимо устанавливать все значения; по умолчанию значений нет.

Если вы не установите `credits` но все же хотите, чтобы поверхность их в вашем приложении, AppKit будет искать файл под названием "Кредиты.html", "Кредиты.rtf", и "Credits.rtfd", в этом порядке, в расслоение возвращается методом NSBundle основной. Первый найденный файл используется, и если его нет, область информации остается пустой. Для получения дополнительной [можно](https://developer.apple.com/documentation/appkit/nsaboutpaneloptioncredits?language=objc) Apple.
...
Включает полноценный режим песочницы в приложении. Это означает, что все рендереры будут запущены в песочнице, независимо от значения флага `sandbox` в WebPreferences.

Этот метод может быть вызван только до того, как приложение будет готово.

### `app.isInApplicationsFolder()` _macOS_

Возвращает `Boolean` - работает ли приложение в настоящее время из папки системы приложений. Использование в сочетании с `app.moveToApplicationsFolder()`
...
Возвращает `Boolean` - был ли этот шаг успешным. Пожалуйста, обратите внимание, если этот шаг будет успешным, ваша заявка будет запущена и перезапущена.

Диалог подтверждения не будет представлен по умолчанию. Если нужно подтверждение операции пользователем, используйте [`dialog`](dialog.md) API.
...
Возвращает `Boolean` - включена `Secure Keyboard Entry` ли она.

По умолчанию этот API вернется `false`.

### `app.setSecureKeyboardEntryEnabled(enabled)` _macOS_

* `enabled` Boolean - Включить или отключить `Secure Keyboard Entry`

Установите `Secure Keyboard Entry` включен в приложении.

С помощью этого API важная информация, такая как пароль и другая конфиденциальная информация, может быть предотвращена другими процессами.

Подробнее [документах Apple](https://developer.apple.com/library/archive/technotes/tn2150/_index.html) подробнее apple.

**Примечание:** включить `Secure Keyboard Entry` только тогда, когда это необходимо, и отключить его, когда он больше не нужен.
...
Более подробную [можно увидеть в документах по доступности Chromium](https://www.chromium.org/developers/design-documents/accessibility) для получения более подробной информации. Отключено по умолчанию.
...
`Integer` , которое возвращает количество значков для текущего приложения. Установка графа на `0` будет скрывать значок.

На macOS, установив это с любыми незеро-интегратор показывает на значок дока. На Linux это свойство работает только для пусковой установки Unity.
...
Объект [`Dock`](./dock.md) `| undefined` , который позволяет выполнять действия на значке приложения в списке пользователей, на macOS.
...
В `Boolean` , который, `true` , показывает, что приложение в настоящее время работает под [Rosetta Переводчик окружающей среды](https://en.wikipedia.org/wiki/Rosetta_(software)).

Вы можете использовать это свойство, чтобы побудить пользователей скачать arm64 версию вашего приложения, когда они работают версия x64 под Rosetta неправильно.
