...
Вызывая `app.setAppLogsPath()` без параметра `path` приведет к тому, что эта директория будет установлена на `~/Library/Logs/YourAppName` на _macOS_, и внутри директории `userData` на _Linux_ и _Windows_.
...
* `path` String (optional) _Windows_ - The path to the Electron executable. По умолчанию - `process.execPath`
