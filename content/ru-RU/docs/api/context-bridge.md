...
When `contextIsolation` is enabled in your `webPreferences` (this is the default behavior since Electron 12.0.0), your `preload` scripts run in an "Isolated World".  Вы можете прочитать больше об изоляции контекста и о том, что она влияет [безопасности](../tutorial/security.md#3-enable-context-isolation-for-remote-content) документов.
