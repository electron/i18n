...
* `options` Object (опціонально) 
  ...
  * `webPreferences` Object (опціонально) - Налаштування особливостей веб-сторінки. 
    * `devTools` Boolean (опціонально) - Чи вмикати DevTools. If it is set to `false`, can not use `BrowserWindow.webContents.openDevTools()` to open DevTools. За замовчуванням `true`.
