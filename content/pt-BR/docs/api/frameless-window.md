...
* Nos sistemas operacionais Windows, as janelas transparentes não funcionarão quando o DWM estiver desativado.
* No Linux, os usuários têm que colocar `--enable-transparent-visuals --disable-gpu` em linha de comando para desativar a GPU e permitir que o ARGB faça uma janela transparente, isso é causado por um bug upstream que [canal alfa não funciona em alguns os drivers da NVidia](https://bugs.chromium.org/p/chromium/issues/detail?id=369209) no Linux.
* No Mac, a sombra da janela nativa não será mostrada em uma janela transparente.
