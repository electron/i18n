...
A regra básica é: se um módulo for [GUI](https://en.wikipedia.org/wiki/Graphical_user_interface) ou relacionado ao sistema de baixo nível, então deve estar disponível apenas no processo principal. You need to be familiar with the concept of [main process vs. processo de renderização ](../tutorial/quick-start.md#main-process) scripts para poder usar esses módulos.
