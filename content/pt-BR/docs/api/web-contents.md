...
* `params` Object
  ...
  * `menuSourceType` String - Input source that invoked the context menu. Pode ser `none`, `mouse`, `keyboard`, `touch`, `touchMenu`, `longPress`, `longTap`, `touchHandle`, `stylus`, `adjustSelection` ou `adjustSelectionReset`.
