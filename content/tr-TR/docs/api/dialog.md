...
* `options` Nesne 
  ...
  * `properties` String[] (optional) - Contains which features the dialog should use. The following values are supported: 
    ...
    * `promptToCreate` - Prompt for creation if the file path entered in the dialog does not exist. Aslında bu yolda dosya oluşturmaz, ancak uygulama tarafından oluşturulması gereken eksik yolların iadesi yapılmasına izin verir. *Windows*
