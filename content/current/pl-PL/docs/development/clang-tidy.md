# Używanie clang-tidy w kodzie C++
...
Uruchamianie `clang-tidy` jest raczej powolne - wewnętrznie kompiluje każdy plik i następnie uruchamia kontrole, więc zawsze będzie jakiś czynnik wolniejszy niż kompilacja. While you can use parallel runs to speed it up using the `--jobs|-j` option, `clang-tidy` also uses a lot of memory during its checks, so it can easily run into out-of-memory errors. As such the default number of jobs is one.
