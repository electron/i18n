...
If you want to support displays with different DPI densities at the same time, you can put images with different sizes in the same folder and use the filename without DPI suffixes. Na przykład:
...
To mark an image as a template image, its filename should end with the word `Template`. Na przykład:
