...
WebView is a custom element (`<webview>`) that will only work inside Electron. Ze worden als een "achterhaald iframe" geïmplementeerd. This means that all communication with the `<webview>` is done asynchronously using IPC. The `<webview>` element has many custom methods and events, similar to `webContents`, that provide you with greater control over the content.
