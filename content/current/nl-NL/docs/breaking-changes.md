...
## Geplande Breaking API Wijzigingen (13.0)

### Verwijderd: `shell.moveItemToTrash()`

De verouderde synchrone `shell.moveItemToTrash()` API is verwijderd. Gebruik in plaats daarvan de asynchrone `shell.trashItem()`.

```js
// Verwijderd in Electron 13
shell.moveItemToTrash(pad)
// Vervang door
...
### Verwijderd: Pepper Flash ondersteuning

Chromium heeft ondersteuning voor Flash verwijderd, dus we moeten het voorbeeld volgen. Zie Chromium's [Flash Roadmap](https://www.chromium.org/flash-roadmap) voor meer details.

### Standaard gewijzigd: `contextIsolatie` standaard `true`

In Electron 12, zal `contextIsolatie` standaard worden ingeschakeld.  Om het vorige gedrag, `contextIsolation: false` te herstellen moet worden opgegeven in WebVoorkeuren.

We [aanbevelen contextIsolatie ingeschakeld te hebben](https://github.com/electron/electron/blob/master/docs/tutorial/security.md#3-enable-context-isolation-for-remote-content) voor de veiligheid van je applicatie.

Voor meer details: https://github.com/electron/electron/issues/23506
...
### Standaard gewijzigd: `crashReporter.start({ compress: true })`

De standaardwaarde van de `comprimeer` optie voor `crashReporter.start` is veranderd van `false` naar `true`. Dit betekent dat crash dumps geüpload zullen worden naar de crash-instion server met de `Content-Encoding: gzip` header, en het lichaam zal worden gecomprimeerd.

Als uw crash inname server geen gecomprimeerde payload ondersteunt, u kunt compressie uitschakelen door `{ compress: false }` in de crash reporter opties op te geven.

### Verouderd: `externe` module

De `externe` module wordt niet meer ondersteund in Electron 12, en wordt verwijderd in Electron 14. Het wordt vervangen door de [`@electron/remote`](https://github.com/electron/remote) module.

```js
// Verouderd in Electron 12:
...
### Afgekeurd: `shell.moveItemToTrash()`

De synchrone `shell.moveItemToTrash()` is vervangen door de nieuwe, asynchrone `shell.trashItem()`.
...
Er zijn geen ingrijpende wijzigingen gepland voor 11.0.
...
### Afgekeurd: `crashReporter.start({ compress: false })`

`{ compress: false }` in `crashReporter.start` wordt niet meer ondersteund. Bijna alle crashinstion servers ondersteunen gzi-compressie. Deze optie wordt verwijderd in een toekomstige versie van Electron.
...
Chromium has removed support for changing the layout zoom level limits, and it is beyond Electron's capacity to maintain it. De functie werd verouderd in Electron 8.x en is verwijderd in Electron 9.x. De zoom van de lay-out limieten zijn nu vastgesteld op een minimum van 0. 5 en maximaal 5.0, zoals gedefinieerd [hier](https://chromium.googlesource.com/chromium/src/+/938b37a6d2886bf8335fc7db792f1eb46c65b2ae/third_party/blink/common/page/page_zoom.cc#11).
...
const getGuestForWebContents = (webContentsId, contents) => {
  ...
  if (gast. ostWebInhoud ! = contents) {
    throw new Error('Access not to webContents')
  }
  return guest
}
...
### API veranderd: `powerMonitor.querySystemIdleTime` is nu `powerMonitor.getSystemIdTime`
