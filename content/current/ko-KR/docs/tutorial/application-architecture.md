...
이미 추측할 수 있듯이, 원격 콘텐츠를 로드하려고 시도한 경우 이것은 보안에 중요한 영향을 미칩니다. 원격 콘텐츠 로드에 대한 더 많은 정보와 지침을  [security documentation](./security.md)에서 찾을 수 있습니다.
...
Node.js 모듈의 대부분은 네이티브가 _아닙니다_. ~650,000 모듈중 400모듈만 네이티브입니다. 그러나, 네이티브 모듈을 사용해야 한다면, [this guide on how to recompile them for Electron](./using-native-node-modules.md)를 참조하십시오.
