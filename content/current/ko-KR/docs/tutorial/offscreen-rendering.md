...
GPU 가속 렌더링은 GPU가 합성에 사용됨을 의미합니다. Because of that, the frame has to be copied from the GPU which requires more resources, thus this mode is slower than the Software output device. 이 모드의 이점은 WebGL 및 3D CSS 애니메이션이 지원된다는 것입니다.
