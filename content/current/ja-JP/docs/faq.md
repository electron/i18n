...
あるいは、Electron が提供する IPC プリミティブを使用することもできます。 メインプロセスとレンダラープロセス間でデータを 共有する [`ipcMain`](api/ipc-main.md) と [`ipcRenderer`](api/ipc-renderer.md) モジュールを使用できます。 ウェブページ間で直接通信するには、[`MessagePort`](https://developer.mozilla.org/en-US/docs/Web/API/MessagePort) を大抵はメインプロセス経由で一方から他方に送信し、 [`ipcRenderer.postMessage()`](api/ipc-renderer.md#ipcrendererpostmessagechannel-message-transfer) を使用します。 その後のメッセージポートを介した通信は直接のものになり、メインプロセスには迂回しません。
