...
`navigator.mediaDevices.getUserMedia` не працює в macOS через фундаментальне обмеження, через яке додаткам, які бажають отримати доступ до звуку системи, потрібне [ підписане розширення ядра](https://developer.apple.com/library/archive/documentation/Security/Conceptual/System_Integrity_Protection_Guide/KernelExtensions/KernelExtensions.html). Chromium, and by extension Electron, does not provide this.
