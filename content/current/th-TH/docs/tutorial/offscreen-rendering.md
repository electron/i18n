...
GPU accelerated rendering means that the GPU is used for composition. Because of that, the frame has to be copied from the GPU which requires more resources, thus this mode is slower than the Software output device. ประโยชน์ของสิ่งนี้ โหมดคือรองรับการสร้างภาพเคลื่อนไหว WebGL และ 3D CSS
