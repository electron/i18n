...
Send an asynchronous message to the renderer process via `channel`, along with arguments. 引數們會用[Structured Clone 演算法](https://developer.mozilla.org/en-US/docs/Web/API/Web_Workers_API/Structured_clone_algorithm)將其序列化，如同 [`postMessage`][]，因此，原型鏈將不會參與其中。 傳遞任何 Function、Promise、Symbol、WeakMap 或 WeakSet 物件將會拋出錯誤。
