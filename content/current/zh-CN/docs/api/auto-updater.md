...
`autoUpdater`是一个[EventEmitter](https://nodejs.org/api/events.html#events_class_eventemitter).
...
* `选项` 对象
  ...
  * `serverType` String (可选) _macOS_ - 可以是`json`或者`default`，有关更多信息请参考 [Squirrel.Mac](https://github.com/Squirrel/Squirrel.Mac) 的自述文件(README)。
