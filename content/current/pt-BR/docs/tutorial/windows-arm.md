...
Se seu aplicativo é executado com Electron 6.0.8 ou mais tarde, agora você pode construí-lo para o Windows 10 no Arm. Isto melhora consideravelmente o desempenho, mas requer a recompilação de todos os módulos nativos usados no seu aplicativo. Também pode exigir pequenas correções para sua compilação e scripts de embalagem.
...
Um monte de código específico do Windows contém se... outra lógica que seleciona entre as arquiteturas x64 ou x86.
...
Por padrão, `node-gyp` desbloqueia os cabeçalhos dos nós do Electron e baixa as versões x86 e x64 do nó `. ib` em `%APPDATA%\. \Local\node-gyp\Cache`, mas não baixa a versão arm64 ([uma correção para isso está em desenvolvimento](https://github.com/nodejs/node-gyp/pull/1875). Para consertar isto:
