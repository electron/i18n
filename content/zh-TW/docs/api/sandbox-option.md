...
> 建立可在 Chromium OS 沙箱中執行的瀏覽器視窗以及畫面轉譯器。 啟用這個選項時，畫面轉譯器必需透過 IPC 與主處理序溝通才能存取 Node.js API。 However, in order to enable the Chromium OS sandbox, electron must be run with the `--enable-sandbox` command line argument.
