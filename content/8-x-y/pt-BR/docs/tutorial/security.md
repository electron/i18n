...
Enquanto uma funcionalidade experimental do Electron, o isolamento de contexto adiciona uma camada adicional de segurança. Cria um novo mundo JavaScript para as APIs do Electron e pré-carrega scripts, que mitiga os chamados ataques "Poluição Protótipo".
...
// Defina uma variável na página antes de carregar
webFrame.executeJavaScript('window. oo = "pé"; )

// A página carregada não será capaz de acessar isso, ela só está disponível
// nesta janela de contexto
. ar = 'bar'

documento. ddEventListener('DOMContentLoaded', () => {
  // Desconectará 'undefined' desde a janela. oo está disponível apenas no console principal
  // contexto
  . og(window.foo)

  // Vai deslogar 'bar' pois window.bar está disponível neste contexto
...
[`webContents`][web-contents] will emit the [`new-window`][new-window] event before creating new windows. Esse evento será passado, entre outros parâmetros, a `url` da janela foi solicitada para abrir e as opções usadas para criá-la. Recomendamos que você use o evento para analisar a criação de janelas, limitando ao que você precisa.
...
```js
// Ruim se o renderizador pode executar conteúdo não confiável
const mainWindow = new BrowserWindow({})
```
...
```html<!-- Ruim se o renderizador pode executar conteúdo não confiável --><webview src="page.html"></webview><!-- Bom --><webview enableremotemodule="false" src="page.html"></webview>
```
