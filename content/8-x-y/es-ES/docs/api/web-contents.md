...
Emitido cuando `remote.getBuiltin()` se llama en el proceso de renderizado. Llamando `event.preventDefault()` evitará que se devuelva el modulo. El valor personalizado puede ser devuelto por la configuración `event.returnValue`.
