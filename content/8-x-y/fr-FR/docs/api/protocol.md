...
Registering a scheme as standard will allow access to files through the [FileSystem API][file-system-api]. Sinon, le moteur de rendu lancera une erreur de sécurité pour le schéma.
...
Lorsque `callback` est appelé avec rien, un numéro ou un objet qui a un `erreur` propriété, la `demande` échouera avec le `erreur` numéro que vous spécifié. For the available error numbers you can use, please see the [net error list][net-error].
