...
Debug 相關的旗標，細節可參考[Debug 主處理序][debugging-main-process]導引。
...
Gives the per-module maximal V-logging levels to override the value given by `--v`. 例如: `my_module=2,foo*=3` would change the logging level for all code in source files `my_module.*` and `foo*.*`.

Any pattern containing a forward or backward slash will be tested against the whole pathname and not only the module. 例如: `*/foo/bar/*=2` would change the logging level for all code in the source files under a `foo/bar` directory.
