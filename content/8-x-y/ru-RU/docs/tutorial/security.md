...
Пока экспериментальная функция Electron, контекст добавляет дополнительного уровня безопасности. Он создает новый мир JavaScript для Electron API и сценарии предварительной загрузки, которые смягчают так называемые атаки "Прототип загрязнения".
...
[`webContents`][web-contents] will emit the [`new-window`][new-window] event before creating new windows. Это событие будет передано, среди других параметров, `url` было предложено открыть окно и опции создать его. Мы рекомендуем использовать событие для анализа создания , ограничивая его только тем, что вам нужно.
