...
The `ipcMain` module is an [Event Emitter][event-emitter]. Bila digunakan dalam proses utama, ia menangani pesan asinkron dan sinkron yang dikirim dari proses renderer (halaman web). Pesan yang dikirim dari penyaji akan dipancarkan ke modul ini.
...
<h3 spaces-before="0"><code>ipcMain.handle(channel, listener)`</h3> 
  * ` saluran </ 0>  String</li>
<li><code>listener` Function<Promise<void> | any> 
...
<p spaces-before="0">Removes any handler for <code>channel`, if present.</p> 
