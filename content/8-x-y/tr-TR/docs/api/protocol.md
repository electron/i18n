...
Registering a scheme as standard will allow access to files through the [FileSystem API][file-system-api]. Aksi takdirde rendercı şema için güvenlik hatası verir.
...
`callback` hiçbir şeyle, bir sayıyla ya da `error` özelliği olan bir nesneyle çağırıldığı zaman `request` belirttiğiniz ` error` numarası ile başarısız olacaktır. For the available error numbers you can use, please see the [net error list][net-error].
