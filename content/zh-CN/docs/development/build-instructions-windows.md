...
* Visual Studio 2017 15.7.2 或更高版本 - [免费下载 VS 2019 社区版](https://www.visualstudio.com/vs/)
  ...
  * 如果您的 Visual Studio 安装在非默认目录中， 您需要 设置几个环境变量来将工具链指向您的安装路径。
    ...
    * `WINDOWSSDKDIR = DRIVE:\path\to\Windows Kits\10`，用 Windows 套件打开的驱动器替换 `DRIVE:` 。 一般情况下将会是`C:`。
  * [Python for Windows (pywin32) 扩展](https://pypi.org/project/pywin32/#files)对于构建流程也是必需的。
* [Node.js](https://nodejs.org/download/)
* [Git](https://git-scm.com)
* 如果您计划 创建完整分布，则 Windows SDK 窗口的调试工具 10.0.15063.468，因为 `symstore.exe` 用于从 `.pdb` 文件创建符号 存储。
...
Windows 安全性不喜欢 Chromium 源代码 中的文件之一（见 https://crbug.com/441184），因此它会不断删除它，导致 `gclient sync` 问题。 您可以按照以下说明</a>，通过

将源树排除在 Windows 安全监控之外。</p> 
...
这可能发生在生成过程中，当使用 Windows 驱动程序套件安装Windows的调试工具时。 卸载 Windows 驱动程序套件并安装上述步骤的调试工具。
