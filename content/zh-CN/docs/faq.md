...
在两个网页（渲染进程）间共享数据最简单的方法是使用浏览器中已经实现的 HTML5 API。 优秀的候选人是 [存储API][storage]， [`localStorage`][local-storage]， [`sessionStorage`][session-storage]，和 [索引DB][indexed-db]。
...
效果仅可见于（某些？ 液晶屏。 即使您没有看到不同的情况，您的一些用户可能也会看到。 最好始终以这种方式确定背景，除非你有理由不这样做。
