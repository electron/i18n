...
> #### 题外话：进程间通讯
> 
> 在 Electron 中, 我们有几种方法可以在主进程和渲染进程之间进行通信。 例如[`ipcRenderer`](../api/ipc-renderer.md)和[`ipcMain`](../api/ipc-main.md)模块间发送消息，可以使用[远程](../api/remote.md)模块那样的RPC通信方式。 这里也有一个常见问题解答[web页面间如何共享数据](../faq.md#how-to-share-data-between-web-pages)。
