...
Electron有API来配置Windows任务栏中的应用程序图标。 此 API 支持仅包含 Windows 的功能，如 [创建 `JumpList`](#jumplist)、 [自定义缩略图和工具栏](#thumbnail-toolbars)、 [图标叠加](#icon-overlays-in-taskbar)以及所谓的 ["闪存框架"效果](#flash-frame)，以及跨平台功能 ，如 [最近的文档][recent-documents] 和 [应用进度][progress-bar]。

## 弹出列表

Windows 允许应用程序自定义一个菜单栏，当用户右键单击任务栏中的应用图标可以看到该菜单栏。 该上下文菜单被成为 `弹出列表`. 您指定 `Tasks` 类别的"跳列表"中的自定义操作， 引用 [MSDN][msdn-jumplist]：
...
与 macOS 中的基座菜单（这是一个真正的菜单）不同，Windows 中的用户任务 像应用程序快捷方式一样工作。 例如，当用户单击任务时，程序 将执行指定参数。

要为您的应用程序设置用户任务，您可以使用 [应用程序][setusertaskstasks] 。
...
要清除任务列表，您需要在 `main.js` 文件中使用空 阵列呼叫 `app.setUserTasks` 。
...
要在应用程序中设置缩略图工具栏，您需要使用 [浏览器窗口][setthumbarbuttons]。
...
赢
  ...
  。
    图标： path. join （__dirname， "按钮 1.png"），
    单击 （） { 控制台.log （"点击按钮1"） [
  ]， {
    太轻： "按钮2"，
    图标： path.join （__dirname， "button2.png"），
    标志： [启用'， '解约点击']，
    点击 （） { 控制台.log （"按钮2 点击" 。 [
  [
]）
...
要为窗口设置覆盖图标，您需要使用 [浏览器窗口][setoverlayicon] 。
...
据 [MSDN][msdn-flash-frame]报道：

> 通常, 会闪现一个窗口, 通知用户该窗口需要注意, 但是该窗口当前没有键盘焦点。

要闪烁浏览器窗口任务栏按钮，您需要使用 [浏览器窗口。闪存框架][flashframe] API。
