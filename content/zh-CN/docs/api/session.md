...
Returns `Session` - 根据`partition`字符串产生的session实例。 When there is an existing `Session` with the same `partition`, it will be returned; otherwise a new `Session` instance will be created with `options`.
