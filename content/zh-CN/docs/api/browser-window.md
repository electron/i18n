...
* `options` Object (可选)
  ...
  * `webPreferences` Object (可选) - 网页功能设置。
    ...
    * `sandbox` Boolean (可选)-如果设置该参数, 沙箱的渲染器将与窗口关联, 使它与Chromium OS-level 的沙箱兼容, 并禁用 Node. js 引擎。 它与 `nodeIntegration` 的选项不同，且预加载脚本的 API 也有限制. Read more about the option [here](../tutorial/sandbox.md).
