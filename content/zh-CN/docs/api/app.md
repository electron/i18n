...
即。 如果您的过程是 应用程序的主要实例，并且您的应用应该继续加载，则此方法会 `true` 返回。  如果当它返回 `false`如果你的程序没有取得锁，它应该立刻退出，并且将参数发送给那个已经取到锁的进程。
...
释放所有由 `requestSingleInstanceLock`创建的锁。 这将 允许应用程序的多个实例再次并排运行。
...
创建一个 ` NSUserActivity ` 并将其设置为当前活动。 活动 有资格 [交接][handoff] 到另一个设备之后。
...
将当前 [移交][handoff] 用户活动标记为不活动，而不将其无效。
...
默认情况下，如果 GPU 处理崩溃过于频繁，Chromium 会禁用 3D ABI（例如 WebGL），直到每个 域名重新启动。 此功能 禁用该行为。
...
返回 [`ProcessMetric[]`](structures/process-metric.md)：与与应用关联的所有过程的内存和 CPU 使用统计的 `ProcessMetric` 对象阵列。
...
**注意：** 此信息仅在 `gpu-info-update` 事件发布后才可用。
...
对于 `infoType` 等于 `complete`： 承诺是履行与 `Object` 包含所有的GPU信息，如在 [铬的GPUInfo对象](https://chromium.googlesource.com/chromium/src/+/4178e190e9da409b055e5dff469911ec6f6b716f/gpu/config/gpu_info.cc)。 这包括 `chrome://gpu` 页面上显示的版本和驱动程序信息。
...
为当前应用设置计数器徽章。 将计数设置为 `0` 将隐藏 徽章。

在 macOS 上，它显示在基座图标上。 在Linux上，它只适用于统一发射器。

**注：** Unity发射器需要存在一个 `.desktop` 文件才能工作， 了解更多信息请阅读 [桌面环境集成][unity-requirement]。
...
要与使用松鼠</a>的 Windows 上的 Electron `autoUpdater` 配合使用， 您需要设置"更新.exe"的启动路径，并通过指定您的 应用程序名称的参数。 例如：</p> 
...
**注意：** 渲染辅助功能树可以显著影响应用的性能。 默认情况下不应启用该功能。



### `应用程序。显示约面板（）`

显示应用程序的面板选项。 这些选项可以用 `app.setAboutPanelOptions(options)`来推翻。
...
如果您没有设置 `credits` 但仍希望在应用中显示它们，AppKit 将在 NSBundle 类方法主返回的捆绑包中查找名为"信用.html"、"信用.rtf"和"信用.rtfd"的文件。 已找到的第一个文件已使用，如果未找到，则信息区域将留空。 有关更多信息，请参阅 Apple [文档](https://developer.apple.com/documentation/appkit/nsaboutpaneloptioncredits?language=objc) 。
...
在应用程序上启用完全沙盒模式。 这意味着所有渲染器都将启动沙盒，无论 WebPreence 中 `sandbox` 标志的价值如何。
...
返回 `Boolean` - 应用程序当前是否从 系统应用程序文件夹运行。 与 `app.moveToApplicationsFolder()`一起使用
...
返回 `Boolean` - 移动是否成功。 请注意，如果 移动成功，您的应用程序将退出并重新启动。

默认情况下不会提供确认对话。 如果您希望允许 用户确认操作，您可以使用 [`dialog`](dialog.md) API 进行确认。

**注意:**如果并非是用户造成操作失败，这个方法会抛出错误。 例如，如果用户取消了授权会话，这个方法将返回false。 如果无法执行复制操作, 则此方法将抛出错误。 错误中的信息应该是翔实的，并告诉 你到底出了什么问题。

默认情况下，如果应用程序目录中存在与移动的应用同名的应用，并且 __ 运行，则现有应用将被丢弃，活动应用将移入其位置。 如果它 __ 运行，预先存在的运行应用程序将承担焦点，以前活动的应用程序将退出自己。 此行为可以通过提供可选冲突处理程序来更改，处理程序返回的 boolean 将决定移动冲突是否通过默认行为得到解决。  即返回 `false` 将确保不采取进一步行动，返回 `true` 将导致默认行为和方法继续。
...
这意味着，如果应用已经存在于用户名录中，如果用户选择"继续移动"，则该功能将继续其默认行为，现有应用将被丢弃，活动应用将移动到其位置。
...
通过使用此 API，可以防止密码和其他敏感信息等重要信息被其他过程截获。

有关更多 详细信息，请参阅 apple 文档</a> 。</p> 
...
如果启用 Chrome 的辅助功能支持，则 `Boolean` 属性 `true` ，否则 `false` 。 如果检测到辅助技术（如屏幕读取器）的使用，则此属性将 `true` 。 将此属性设置为手动 `true` 可手动启用 Chrome 的辅助功能支持，允许开发人员在应用程序设置中向用户暴露无障碍切换。

有关详细信息，请参阅 chromium 的辅助文档</a> 。 默认为禁用</p> 
...
`Menu | null` 财产，如果设置 [`Menu`](menu.md) ，则返回，否则 `null` 。 用户可以通过 [菜单](menu.md) 来设置此属性。



### `app.badgeCount` _· 利努克斯 ·_ _· 马科斯 ·_

返回当前应用徽章计数的 `Integer` 属性。 将计数设置为 `0` 将隐藏徽章。

在 macOS 上，将此设置为坞图标上显示的任何非零整数。 在Linux上，此属性仅适用于统一发射器。

**注：** Unity发射器需要存在一个 `.desktop` 文件才能工作， 了解更多信息请阅读 [桌面环境集成][unity-requirement]。

**注意：** 在 macOS 上，您需要确保您的应用程序具有显示此属性生效通知的权限 。



### `app.commandLine` _·里德利·_

[`CommandLine`](./command-line.md) 对象，允许您读取和操作 Chromium 使用的 命令行参数。



### `app.dock` _马科斯_ _只_

一个 [`Dock`](./dock.md) `| undefined` 对象，允许您在 macOS 上的用户 坞中对应用图标执行操作。
...
表示当前应用程序名称的 `String` 属性，这是应用程序 `package.json` 文件中的名称。
...
用户代理字符串 Electron 的 `String` 将用作全球回退。

当没有将用户代理设置在 `webContents` 或 `session` 级别时，将使用此用户代理。  它有助于确保您的整个 应用程序具有相同的用户代理。  在应用初始化中尽早设置为自定义值 ，以确保使用您的覆盖值。



### `应用。允许伦德程序重复使用`

`Boolean` ，当 `true` 禁用电子已经到位的覆盖 ，以确保每次导航重新启动渲染器过程。  此属性的当前 默认值为 `true`。

目的是让这些覆盖在默认情况下成为禁用，然后在 在未来的某个时候，此属性将被删除。  此属性会影响 您可以在渲染器过程中使用哪些本机模块。  有关 Electron 重新启动渲染器过程的方向 以及渲染器过程中 本机模块的使用情况，请查看此 [跟踪问题](https://github.com/electron/electron/issues/18397)。



### `app.runningUnderRosettaTranslation` _马科斯_ _只_

一 `Boolean` ，当 `true` 表明该应用程序目前正在运行 下 [罗塞塔翻译环境](https://en.wikipedia.org/wiki/Rosetta_(software))。

您可以使用此属性来提示用户下载 应用程序的 arm64 版本，当他们在罗塞塔 下运行 x64 版本不正确。
