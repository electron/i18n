...
* `icon` String(optional) - 设置快捷方式的图标的路径 `icon` 和`iconIndex` 必须一起设置。 默认值为空，它使用 目标的图标。
* `iconIndex` 编号（可选） - 当 `icon` 是 DLL 或 EXE 时图标的资源 ID。 默认值为0。
* `appUserModelId` String (optional) - 使用者模式ID 默认为空值。
* `toastActivatorClsid` 字符串（可选） - 应用程序烤面包激活器 CLSID。 参与行动中心需要 。
